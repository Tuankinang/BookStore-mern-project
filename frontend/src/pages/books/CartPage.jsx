...
import { FiArrowLeft, FiMinus, FiPlus, FiShield, FiTag, FiTrash2, FiTruck } from "react-icons/fi";
...
const CartPage = () => {
  ...
  const handleQuantityObj = (type, _id) => {
    if (type === 'increment') {
        // Tìm sản phẩm trong giỏ để check tồn kho
  ...
  // 2. Xử lý nhập số trực tiếp (Có check tồn kho)
  const handleInputChange = (e, _id) => {
    const value = parseInt(e.target.value);
    
    if (value > 0) {
        const item = cartItems.find(i => i._id === _id);
        // Nếu nhập quá số lượng tồn kho -> Chặn và set về max stock
  ...
  const handleCheckout = () => {
    ...
    if (invalidItems.length > 0) {
        const itemNames = invalidItems.map(item => `- ${item.title} (Còn: ${item.stock}, Bạn chọn: ${item.quantity})`).join('\n');
      ...
      return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] bg-gray-50 text-center px-4">
            <div className="bg-white p-8 rounded-2xl shadow-sm max-w-md w-full">
                ...
                <p className="text-gray-500 mb-8">Có vẻ như bạn chưa thêm cuốn sách nào vào đây cả.</p>
                <Link to="/" className="inline-flex items-center justify-center px-6 py-3 border border-transparent text-base font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 w-full transition-colors">
    ...
    <div className="bg-gray-100 h-auto py-10 px-4 sm:px-6 lg:px-8 font-sans rounded-xl">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-8">
                <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-200">
                    ...
                    <div className="flex justify-between items-center p-6 border-b border-gray-100 bg-gray-50">
                        <span className="font-semibold text-gray-700">Chi tiết sản phẩm</span>
                        <button onClick={handleClearCart} className="text-red-500 hover:text-red-700 text-sm font-medium flex items-center gap-1 transition-colors">
                            <FiTrash2 /> Xóa tất cả
                        </button>
                    </div>
                            ...
                            <li key={item._id} className="p-6 flex flex-col sm:flex-row items-center gap-6 hover:bg-gray-50 transition-colors">
                                ...
                                <div className="flex-1 w-full text-center sm:text-left">
                                    <h3 className="text-lg font-bold text-gray-900 mb-1">
                                        <Link to={`/books/${item._id}`} className="hover:text-indigo-600 transition-colors">{item.title}</Link>
                                    </h3>
                                    <p className="text-sm text-gray-500 mb-2 capitalize">Thể loại: {item.category}</p>
                                    <p className={`text-xs font-bold inline-block px-2 py-0.5 rounded ${item.quantity > item.stock ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
                                        {item.quantity > item.stock ? `Không đủ hàng (Kho: ${item.stock})` : `Còn hàng: ${item.stock}`}
                                    </p>
                                </div>
                                ...
                                <div className="text-right min-w-[80px] flex flex-col items-center sm:items-end gap-2">
                                    <p className="text-lg font-bold text-gray-900">${(item.newPrice * item.quantity).toFixed(2)}</p>
                                    <button 
            ...
            {/* --- CỘT PHẢI: TỔNG KẾT ĐƠN HÀNG (Chiếm 4 phần) --- */}
            <div className="lg:col-span-4">
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sticky top-6">
                    ...
                    {/* Mã giảm giá (Giả lập để lấp đầy khoảng trống) */}
                    <div className="mb-6">
                        <label className="block text-sm font-medium text-gray-700 mb-2">Mã giảm giá</label>
                        <div className="flex gap-2">
                            <input type="text" placeholder="Nhập mã..." className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none" />
                            <button className="bg-gray-100 hover:bg-gray-200 text-gray-800 px-4 py-2 rounded-lg text-sm font-medium transition-colors">Áp dụng</button>
                        </div>
                    </div>
